...
from .models import UserProfile # TODO измените на свою модель
        ...
        if user_profile:
            try:
                ...
                return HttpResponseRedirect(
                    reverse(
                        "127.0.0.1",  # TODO измените на свой адрес
                    )
                )
            user.backend = "django.contrib.auth.backends.ModelBackend"
            login(request, user)
            return HttpResponseRedirect(reverse("127.0.0.1")) # TODO измените на свой адрес
